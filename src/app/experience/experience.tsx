...
const Experience: React.FC = () => {
  return (
    <div id="text-container" className="relative flex flex-col gap-4 w-full lg:w-kic-width h-fit overflow-y-auto pointer-events-none [&_*]:pointer-events-auto">
      {experienceContent.map((experience, index) => (
        <div key={index} className="relative flex flex-col justify-between gap-2 w-full p-4 bg-black/10 rounded-sm transition-colors duration-[250ms] lg:hover:bg-black/20">
          ...
          <div className="w-full h-fit flex flex-row justify-between items-center">
            <h4 className="relative w-fit font-ibm-plex-sans text-base lg:text-lg text-fgContrast font-semibold selection:bg-fgContrast" dangerouslySetInnerHTML={{ __html: experience.position }}></h4>
